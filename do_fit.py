...
def do_signal_region_fit(dataset_params, output_params, fit_params, args, write=True, get_yields=False):
    ...
    if not args.cache:
        ...
        model_pt1 = FitModel({'branch' : b_mass_branch, 'dataset' : dataset_rare})
        model_pt1.add_signal_model('sig_pdf', 'dcb', fit_params.fit_defaults, let_float=True)
        model_pt1.fit_model = model_pt1.sig_pdf

        # Fit model to data
        model_pt1.fit(dataset_rare, printlevel=printlevel)
    ...
    if not args.cache:
        ...
        model_pt2 = FitModel({'branch' : b_mass_branch, 'dataset' : dataset_samesign_data})
        ...
        # Fit model to data
        model_pt2.fit(dataset_samesign_data, printlevel=printlevel)
    ...
    # Part 3 - Fit jpsi leakage in low-q2 region from MC
    if args.verbose:
    ...
    # Build Roofit model for exponential background
    model_pt3 = FitModel({'branch' : b_mass_branch, 'dataset' : dataset_jpsi})
    model_pt3.add_background_model('jpsi_bkg_pdf', 'kde', fit_params.fit_defaults, let_float=False)
    model_pt3.fit_model = model_pt3.jpsi_bkg_pdf

    # Plot fit result
    model_pt3.plot_fit(
        b_mass_branch,
        dataset_jpsi,
        os.path.join(output_params.output_dir,'fit_'+args.mode+'_pt3_jpsi_template.pdf'),
    )

    # Save fit shape parameters
    template = save_params(params, os.path.join(output_params.output_dir,'fit_'+args.mode+'_template.yml'), fit_params, args)

    # Part 4 - Add template for final fit
    if args.verbose:
    ...
    model_pt4 = FitModel({'branch' : b_mass_branch, 'dataset' : dataset_data})
    model_pt4.add_signal_model('sig_pdf', 'dcb', template, let_float=False)
    model_pt4.add_background_model('comb_bkg_pdf', 'exp', template, let_float=False)
    model_pt4.add_background_model('jpsi_bkg_pdf', model_pt3.background_models['jpsi_bkg_pdf'])

    sig_coeff = ROOT.RooRealVar('sig_coeff', 'Signal PDF Coefficient', 0.)
    comb_bkg_coeff = ROOT.RooRealVar('comb_bkg_coeff', 'Combinatorial Background Coefficient', 0.15,0.0, 100000.)
    jpsi_bkg_coeff = ROOT.RooRealVar('jpsi_bkg_coeff', 'J/Psi Leakage Background Coefficient', 0.01,0.0, 100000.)
    model_pt4.fit_model = ROOT.RooAddPdf(
        'pdf_sum_final',
        'Sum of Signal and Background PDFs',
        ROOT.RooArgList(
            model_pt4.sig_pdf,
            model_pt4.comb_bkg_pdf,
            model_pt4.jpsi_bkg_pdf,
        ),
        ROOT.RooArgList(
            sig_coeff,
            comb_bkg_coeff,
    ...
    model_pt4.constraints.update({
        'exp_slope_constraint' : ROOT.RooGaussian('exp_slope_constraint', 'exp_slope_constraint', model_pt4.background_models['comb_bkg_pdf'].exp_slope, ROOT.RooFit.RooConst(template['exp_slope']), ROOT.RooFit.RooConst(0.5))
    })

    # Fit model to data
    fit_range = "sb1,sb2" if fit_params.blinded else 'full'
    fit_norm_range = "sb1,sb2" if fit_params.blinded else 'full'
    model_pt4.fit(dataset_data, fit_range=fit_range, fit_norm_range=fit_norm_range, printlevel=printlevel)
    params = model_pt4.fit_result.floatParsFinal()

    # Plot fit result
    model_pt4.plot_fit(
        b_mass_branch,
        dataset_data,
        os.path.join(output_params.output_dir,'fit_'+args.mode+'_pt4_final.pdf'),
        fit_components = [
            model_pt4.sig_pdf,
            model_pt4.comb_bkg_pdf,
            model_pt4.jpsi_bkg_pdf,
    ...
    jpsi_bkg_pdf_norm = ROOT.RooRealVar('jpsi_bkg_pdf_norm', 'Number of jpsi low-q2 background events', 0, 0, 1000000)

    # Write final fit to RooWorkspace
    if write:
        write_workspace(output_params, args, model_pt4, extra_objs=[comb_bkg_pdf_norm, jpsi_bkg_pdf_norm])
...
def do_jpsi_control_region_fit(dataset_params, output_params, fit_params, args, write=True, get_yields=False):
    ...
    if not args.cache:
        ...
        fit_range = "sb1,sb2" if fit_params.blinded else 'full'
        fit_norm_range = "sb1,sb2" if fit_params.blinded else 'full'
        model_pt2.fit(dataset_data, fit_range=fit_range, fit_norm_range=fit_norm_range, printlevel=printlevel)
        params = model_pt2.fit_result.floatParsFinal()
    ...
    fit_range = "sb1,sb2" if fit_params.blinded else 'full'
    fit_norm_range = "sb1,sb2" if fit_params.blinded else 'full'
    model_pt3.fit(dataset_data, fit_range=fit_range, fit_norm_range=fit_norm_range, printlevel=printlevel)
