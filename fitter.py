...
def fit_new(
  ...
  input_path = "./output/slimmed/slimmed_"+sample+".root"
  ...
  weight = ROOT.RooRealVar("trig_wgt","Weight",-0.1,10.0)
  #bdt2022 = ROOT.RooRealVar("Noah_BDT_baseline","Weight",-100.,100.)
  ...
  if trigger != None and trigger != "":
    ...
    variables = ROOT.RooArgSet(mass,trg,weight)#,bdt2022)
  ...
  defaults = {
      "JPSI":{
          ...
          "cb_alphaR":(2.,0.5,3.),
          "cb_nR":(10.,1.,100.),
          "exp_slope":(-0.5,-100.,20.), # combinatorial
...
#  else:
  ...
  # Binning
  #nbins = 75 if region != "KEE" else 20
  nbins = 100
  if isMC==False:
...
#  color_exp = ROOT.TColor(index_exp, 117./255.,112./255.,179./255.)
  ...
  if ( write_2sig_region_bkg==True or isMC==True ) and trigger != None :
    ...
    dctscan = {}
    filenamescan = './output/scans/scantester_bdtcut_'+"{:.5f}".format(bdtcut)+'.json'
    try:
    ...
    if write_signal_params==True:
      for param in params:
        if param.GetName() in ["signal_num","cb_alphaL","cb_alphaR","cb_mean","cb_nL","cb_nR","cb_sigma"]:
          ...
          #if param.GetName() == "signal_num" and region == "KEE" and isMC: add two sigma for Lowq^2
            #dct["mc"]["KEE"][trigger_str][signal_num.GetName()] = (signal_in2sig,signal_in2sig-signal_num.getError()*signal_window.getVal(),signal_in2sig+signal_num.getError()*signal_window.getVal())
...
#  #legend.AddEntry("","Total Entries =  "+tt,"")
  ...
  # Save canvas
  canvas.SaveAs("./output/plots/fitted_"+sample+"_"+var+str("_"+trigger if trigger is not None else "")+".pdf")
  input_file.Close()
...
# Define region
def region(sample):
  regions = ["KEE","JPSI","PSI2S"]
  _region = None
  if     "KEE" in sample: _region = regions[0] # MC sample
  elif "KEE" in sample: _region = regions[0] # Data sample
