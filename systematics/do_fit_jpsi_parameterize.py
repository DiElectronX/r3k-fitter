...
def jpsi_fit_kde_partial_bkg(dataset_params, output_params, fit_params, args, write=True, get_yields=False, file_label=None, legend_text=None, param_file_lock=False, coeffCon=False, isMinos=False):
    '''
    The standard full fitting with 
        signal: dcb+dcb
        combinatorial: exp
        partial k(0)star: kde *** chaning cfg file for up/down samples of k(0)star
        partial jpsipi_jpsi_pion: dcb NOTE: this have to be refited
        NOTE: can use previously saved template from the dcbdcb_sig yml file (to account for minos) except refitting jpsipi
        NOTE: template needs to be cloned and updated once refitting jpsipi
    '''
    printlevel = set_verbosity(args)
    set_mode(dataset_params, output_params, fit_params, args)
    makedirs(output_params.output_dir)
    ...
    if args.cache:
        # this is not very versatile --> (maybe) making an input template in -lc 

        file_path = 'dcbdcb_sig/' + output_params.output_dir.split('/')[-1] + '/fit_jpsi_template.yml'
        with open(file_path, 'r') as file:
            template = yaml.safe_load(file)

        # Clone to output directory without changing the filename
        clone_path = os.path.join(output_params.output_dir, os.path.basename(file_path))
        os.makedirs(output_params.output_dir, exist_ok=True)

        # Copy content without using shutil
        with open(file_path, 'r') as src, open(clone_path, 'w') as dst:
            dst.write(src.read())
        
        print(f"Cloned template saved to: {clone_path}")
    
    else:
        print('Using the new fit template file')
    ...
    # Fit partial background shape to jpsipi MC
    # refitting becasue of the update up/down scaling of jpsipi_jspi_pion file
    print('\nStarting Fit 4 - JpsiPi Partial Template \n{}'.format(50*'~'))
    # if args.verbose:
    #     print('\nStarting Fit 4 - JpsiPi Partial Template \n{}'.format(50*'~'))
